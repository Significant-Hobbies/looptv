...
function AboutHero({ totalSources, totalVideos }: { totalSources: number; totalVideos: number }) {
  return (
    <section className="mt-6">
      ...
      <p className="mt-3 max-w-prose text-sm leading-6 text-zinc-400">
        ...
        no account, no API keys, no algorithm deciding what&apos;s next. {stations.length} stations,{' '}
        {totalSources} channels, {totalVideos.toLocaleString()} videos in the catalog today.
      </p>
...
export default function AboutPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12 text-zinc-300">
      ...
      <AboutHero totalSources={totalSources} totalVideos={totalVideos} />
      <FeaturesSection features={features} className="mt-12" />
      <HowItWorks />
      <TipsSection />
      <AddYourOwn />
      <ClosingCta />
