      ...
      <div className="relative w-full max-w-2xl mx-4">
        <div className="bg-zinc-900 rounded-xl border border-white/10 shadow-2xl overflow-hidden">
              ...
              {results.map((video, i) => (
                <div
                  ...
                  className={`w-full text-left px-4 py-3 flex items-center gap-3 transition-colors ${
                    i === selectedIndex ? 'bg-white/10' : 'cursor-pointer hover:bg-white/5'
                  }`}
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {onToggleWatchLater && (
                      <button
                        ...
                        title={
                          watchLaterIds?.has(video.id) ? 'Remove from Watch Later' : 'Watch Later'
                        }
                      >
                        <svg
                          className="w-4 h-4"
                          fill={watchLaterIds?.has(video.id) ? 'currentColor' : 'none'}
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path
          ...
          {!query.trim() &&
            (() => {
              const wlVideos =
                watchLaterIds && watchLaterIds.size > 0
